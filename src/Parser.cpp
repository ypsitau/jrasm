...
bool Parser::FeedToken(AutoPtr<Token> pToken)
{
	...
	switch (_stat) {
	case STAT_LineTop: {
		if (pToken->IsType(Token::TYPE_Symbol)) {
			_stat = STAT_Label;
		} else if (pToken->IsType(Token::TYPE_White)) {
			_stat = STAT_Instruction;
		}
		break;
	}
	case STAT_Label: {
		if (pToken->IsType(Token::TYPE_Colon)) {
	...
	case STAT_Operand: {
		if (pToken->IsType(Token::TYPE_White)) {
			// nothing to do
		} else if (pToken->IsType(Token::TYPE_EOL)) {
			_exprStack.pop_back();
			_stat = STAT_LineTop;
		} else if (pToken->IsType(Token::TYPE_Comma)) {
			
		} else if (pToken->IsType(Token::TYPE_Symbol)) {
			_exprStack.back()->AddChild(new Expr_Symbol(pToken->GetString()));
		} else if (pToken->IsType(Token::TYPE_Number)) {
			_exprStack.back()->AddChild(new Expr_Number(pToken->GetNumber()));
		} else if (pToken->IsType(Token::TYPE_Plus)) {
			new Expr_BinOp_Add(nullptr, nullptr);
		} else if (pToken->IsType(Token::TYPE_Minus)) {
			new Expr_BinOp_Sub(nullptr, nullptr);
		} else if (pToken->IsType(Token::TYPE_Asterisk)) {
			new Expr_BinOp_Mul(nullptr, nullptr);
		} else if (pToken->IsType(Token::TYPE_Slash)) {
			new Expr_BinOp_Div(nullptr, nullptr);
		} else if (pToken->IsType(Token::TYPE_BracketL)) {
			Expr *pExpr = new Expr_Bracket();
			_exprStack.back()->AddChild(pExpr);
			_exprStack.push_back(pExpr);
		} else if (pToken->IsType(Token::TYPE_BracketR)) {
			if (!_exprStack.back()->IsType(Expr::TYPE_Bracket)) {
				_tokenizer.SetErrMsg("no opening bracket matched");
			}
			_exprStack.pop_back();
		} else if (pToken->IsType(Token::TYPE_ParenthesisL)) {
			Expr *pExpr = new Expr_Parenthesis();
			_exprStack.back()->AddChild(pExpr);
			_exprStack.push_back(pExpr);
		} else if (pToken->IsType(Token::TYPE_ParenthesisR)) {
